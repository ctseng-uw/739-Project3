...
async def test_primary_dead_simple(
    ...
    assert (await client.read(0, 0))[1] == pad_to_4096("123456")
    await primary.commit_suicide()
    await backup.become_primary()
    await client.write(1, 6, "789")
    assert (await client.read(1, 0))[1] == pad_to_4096("123456789")
...
async def test_primary_dead_up_again(
    ...
    assert (await client.read(1, 4096))[1] == pad_to_4096("banana")
